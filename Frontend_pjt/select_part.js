...
function select_func(id) {
      ...
      if (list_video.status == 200) {
        ...
        for (let i = 0; i < select_video.length; i++) {
          ...
          if (v["part"] == type) {
            ...
            video_add.innerHTML = `
            <a href="./detail.html">
            <img src="https://img.youtube.com/vi/${v["id"]}/mqdefault.jpg" class="card-img-top imgBtn" id=${v["no"]}>
            <div class="card-body">
              <h6 class="card-title">${v["title"]}</h6>
              <p class="card-text"></p>
              <span class="btn btn-secondary">${v["part"]}</span>
            </div>
            <div class="card-footer">
              <small class="text-muted">${v["channelName"]}</small>
            </div>
            `;
            cnt += 1;
            document
  ...
  list_video.open("GET", "data/video.json");
  list_video.send();
}
